    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 max-w-[1600px] mx-auto animate-reveal">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-2xl lg:text-3xl font-bold tracking-tight mt-1 whitespace-pre-wrap">Esta é uma solicitação de implementação de código. Aplique a instrução abaixo gerando os arquivos, componentes, lógica e estilos necessários: Faça uma validação final do Financeiro após a correção dos filtros server-side.

NÃO altere código.

NÃO altere banco.

NÃO faça novas otimizações.

Objetivo: confirmar que a migração dos filtros do frontend para o Supabase não alterou nenhuma regra financeira.
...
Verifique especificamente clientes suspensos:

- cobranças pendentes/atrasadas devem continuar ocultas da visão operacional;

- cobranças pagas devem continuar preservadas;

- reativar o cliente deve fazer as cobranças voltarem;

- nenhum registro deve ser alterado no banco.

Verifique especificamente "Atrasados":

- deve considerar exatamente os mesmos lançamentos que a regra anterior considerava;

- não deve misturar indevidamente atrasados de outros contextos;

- não deve desaparecer lançamentos que deveriam aparecer.

Verifique especificamente "Sem vínculo":

- deve continuar identificando lançamentos sem DME/lote/vínculo conforme a regra existente.
...
- mudança de filtro deve voltar para page 1;

- nenhum registro deve desaparecer apenas por estar em outra página.
...
Eles devem continuar independentes da página atual.
...
- se houve alguma diferença de comportamento;

- se a query server-side reproduz corretamente as regras anteriores;

- se existem riscos de regressão.</h1>
