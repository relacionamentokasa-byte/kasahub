...
import { fetchTransactions, fetchFinanceStats, fetchCategories, updateTransaction } from "@/lib/finance-api";
...
import { TrendingUp, TrendingDown, Wallet, Filter, Download, Plus, Search, CheckCircle2, Clock, AlertCircle, FileSpreadsheet, Upload, Trash2, Settings, Landmark, ChevronLeft, ChevronRight, Pencil, CreditCard, Barcode, Receipt, Home, BadgeCheck, CheckCircle, Ban, } from "lucide-react";
...
const normalize = (s) => (s || "")
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
const isProLabore = (name) => normalize(name) === PRO_LABORE;
const isInvestimento = (name) => normalize(name).includes("investimento");
const formatDateOnlyBR = (date) => date ? new Date(`${date.slice(0, 10)}T00:00:00`).toLocaleDateString("pt-BR") : "";
...
function FinancialPage() {
    ...
    const [baixaTx, setBaixaTx] = useState(null);
    const [deletingTx, setDeletingTx] = useState(null);
    const [boletoTx, setBoletoTx] = useState(null);
    const [reciboTx, setReciboTx] = useState(null);
    const getSignedUrlFn = useServerFn(getBoletoSignedUrl);
    const openBoletoPdf = async (boletoId) => {
        try {
    ...
    const mapType = (raw) => {
        ...
        if (!s)
            return null;
        if (s.includes("receita") || s.includes("entrada") || s.includes("income") || s.includes("credito"))
            return "income";
        if (s.includes("despesa") || s.includes("saida") || s.includes("expense") || s.includes("debito"))
            return "expense";
        return null;
    };
    const mapStatus = (raw) => {
            ...
            complete: async (results) => {
                try {
                    ...
                    if (erroConta || !contas || contas.length === 0) {
                        toast.error("Por favor, cadastre pelo menos uma Conta Bancária no sistema antes de importar a planilha.");
                        inputEl.value = "";
                        return;
                    }
                    ...
                    const payload = rows
                        ...
                        const getCol = (...keys) => {
                            for (const k of keys) {
                                const found = Object.keys(r).find((rk) => rk.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim() === k);
                                if (found && r[found] != null && String(r[found]).trim() !== "")
                                    return r[found];
                            }
                            return null;
                        };
    ...
    const [quickChip, setQuickChip] = useState("none");
    ...
    const statusMut = useMutation({
        mutationFn: ({ id, status }) => {
            ...
            return updateTransaction(id, { status: status, payment_date: status === "paid" ? todayLocal : null });
        },
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["transactions"] });
            qc.invalidateQueries({ queryKey: ["contas_bancarias"] });
    ...
    const bulkBaixaMut = useMutation({
        mutationFn: async (ids) => {
            const { error } = await supabase
                .from("transactions")
                .update({ status: "paid", payment_date: todayStrLocal })
                .in("id", ids);
            if (error)
                throw error;
        },
    ...
    const getCatName = (t) => t.categorias_financeiras?.nome || t.category || "";
    ...
    const filteredTransactions = transactions.filter((t) => {
        if (filter.nfStatus !== "all" && (t.nf_status || "pendente") !== filter.nfStatus)
            return false;
        if (filter.boletoStatus !== "all" && (t.boleto_internal_status || "nao_se_aplica") !== filter.boletoStatus)
            return false;
        if (!showCancelled && t.status === "cancelled")
            return false;
        const matchSearch = t.description.toLowerCase().includes(filter.search.toLowerCase()) ||
            t.clients?.company?.toLowerCase().includes(filter.search.toLowerCase()) ||
            t.clients?.name?.toLowerCase().includes(filter.search.toLowerCase());
        if (!matchSearch)
            return false;
        const proLab = isProLabore(getCatName(t));
        if (quickFilter === "income" && t.type !== "income")
            return false;
        if (quickFilter === "expense_op" && !(t.type === "expense" && !proLab))
            return false;
        if (quickFilter === "pro_labore" && !proLab)
            return false;
        if (quickChip === "today") {
            if ((t.due_date || "").slice(0, 10) !== todayStrLocal)
                return false;
        }
        else if (quickChip === "week") {
            const d = (t.due_date || "").slice(0, 10);
            if (!d || d < todayStrLocal || d > weekFromTodayStr)
                return false;
        }
        else if (quickChip === "overdue") {
            if (t.status !== "pending" || !t.due_date || t.due_date >= todayStrLocal)
                return false;
        }
            ...
            if (t.type === "income") {
                ...
                if (t.client_id || t.supplier_id || t.freelancer_id || t.partner_id)
                    return false;
            }
        }
        return true;
    ...
    const proLaboreMes = transactions
        .filter((t) => t.type === "expense" && isProLabore(getCatName(t)))
        .reduce((acc, t) => acc + Number(t.status === "paid" ? t.amount : (Number(t.valor_previsto) > 0 ? t.valor_previsto : t.amount)), 0);
    ...
    return (<div className="p-4 sm:p-6 lg:p-8 space-y-8 max-w-[1600px] mx-auto animate-reveal">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button className="rounded-full gap-2 bg-primary text-primary-foreground" onClick={() => setTransactionOpen(true)}>
      ...
      <div className="flex items-center justify-center gap-6 bg-surface border border-border rounded-2xl p-4 shadow-sm animate-reveal">
        <Button variant="ghost" size="icon" onClick={prevMonth} className="rounded-full hover:bg-primary/10 hover:text-primary transition-all">
          <ChevronLeft className="size-6"/>
        </Button>
        
        <Popover>
          <PopoverTrigger asChild>
            <button type="button" className="flex flex-col items-center min-w-[200px] rounded-xl px-4 py-1 hover:bg-primary/5 transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/30" aria-label="Selecionar mês e ano">
          ...
          <PopoverContent className="w-auto p-3 pointer-events-auto" align="center">
            <div className="flex gap-2">
              <Select value={String(selectedDate.getMonth())} onValueChange={(v) => {
            ...
            setSelectedDate(d);
                ...
                <SelectContent>
                  {["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"].map((m, i) => (<SelectItem key={i} value={String(i)}>{m}</SelectItem>))}
                </SelectContent>
              </Select>
              <Select value={String(selectedDate.getFullYear())} onValueChange={(v) => {
            const d = new Date(selectedDate);
            ...
            setSelectedDate(d);
                ...
                <SelectContent>
                  {Array.from({ length: 11 }, (_, i) => new Date().getFullYear() - 5 + i).map((y) => (<SelectItem key={y} value={String(y)}>{y}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          </PopoverContent>
        ...
        <Button variant="ghost" size="icon" onClick={nextMonth} className="rounded-full hover:bg-primary/10 hover:text-primary transition-all">
      ...
      <FinancialRulesPanel totalFaturamento={stats?.recebidasReceitas ?? 0} faturamentoPrevisto={(stats?.previstasReceitas ?? 0) + (stats?.recebidasReceitas ?? 0)} despesasReais={despesasReaisOperacionais} investimentoRealizado={investimentoRealizado} periodoLabel={currentMonthLabel}/>

      {/* Quick Filter Tabs */}
      <div className="flex flex-wrap items-center gap-2">
        <ToggleGroup type="single" value={quickFilter} onValueChange={(v) => v && setQuickFilter(v)} className="bg-surface border border-border rounded-full p-1 gap-1">
        ...
        {cancelledCount > 0 && (<button type="button" onClick={() => setShowCancelled((v) => !v)} className={cn("ml-auto inline-flex items-center gap-2 text-xs px-3 h-9 rounded-full border transition-colors", showCancelled
                ? "border-foreground/20 bg-foreground/5 text-foreground/70"
                : "border-border text-foreground/50 hover:text-foreground hover:border-foreground/30")} title={showCancelled ? "Ocultar cancelados" : "Mostrar cancelados"}>
      ...
      <div className="bg-surface border border-border rounded-2xl p-4 flex flex-wrap gap-4 items-end shadow-sm">
        <div className="flex-1 min-w-[200px] space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input placeholder="Descrição ou cliente..." className="pl-9 h-10 rounded-xl" value={filter.search} onChange={e => setFilter({ ...filter, search: e.target.value })}/>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(() => {
            const missingLinksCount = transactions.filter((t) => {
                if (t.status === "cancelled")
                    return false;
                if (t.type === "income")
                    return !t.client_id;
                if (t.type === "expense")
                    return !t.client_id && !t.supplier_id && !t.freelancer_id && !t.partner_id;
                return false;
            }).length;
            const chips = [
                { id: "none", label: "Tudo" },
            ...
            return chips.map((c) => {
                ...
                return (<button key={c.id} type="button" onClick={() => setQuickChip(c.id)} className={cn("text-xs px-3 h-8 rounded-full border transition-colors font-medium", active
                        ? c.id === "overdue"
      ...
      {/* Bulk action bar */}
      {selectedIds.size > 0 && (<div className="sticky top-2 z-20 flex flex-wrap items-center gap-3 bg-primary text-primary-foreground rounded-2xl px-4 py-3 shadow-lg animate-reveal">
          <span className="text-sm font-semibold">
            {selectedIds.size} selecionado(s)
          </span>
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <Button size="sm" variant="secondary" className="rounded-full gap-1.5 h-8" onClick={() => bulkBaixaMut.mutate(Array.from(selectedIds))} disabled={bulkBaixaMut.isPending}>
            ...
            <Popover open={bulkCategoryOpen} onOpenChange={setBulkCategoryOpen}>
              ...
              <PopoverContent align="end" className="w-64 p-2">
                ...
                <div className="max-h-64 overflow-auto space-y-0.5">
                  {categories.map((c) => (<button key={c.id} type="button" onClick={() => bulkCategoryMut.mutate({
            ...
            <Button size="sm" variant="destructive" className="rounded-full gap-1.5 h-8" onClick={() => {
            ...
            <Button size="sm" variant="ghost" className="rounded-full h-8 text-primary-foreground/80 hover:text-primary-foreground hover:bg-primary-foreground/10" onClick={() => setSelectedIds(new Set())}>
              Limpar
            </Button>
          </div>
        </div>)}
      ...
      <div className="bg-surface border border-border rounded-2xl overflow-hidden shadow-sm">
        <Table>
          <TableHeader className="bg-muted/30">
            <TableRow>
              <TableHead className="w-10 py-4">
                <Checkbox checked={filteredTransactions.length > 0 &&
            filteredTransactions.every((t) => selectedIds.has(t.id))} onCheckedChange={(c) => {
            if (c) {
          ...
          <TableBody>
            {isLoading ? (<TableRowsSkeleton rows={6} columns={9}/>) : filteredTransactions.length === 0 ? (<TableRow>
              ...
              </TableRow>) : (filteredTransactions.map((t) => {
            const isSuspended = t.clients?.financial_collection_status === 'suspended';
            ...
            const effectiveStatus = t.status === "pending" && t.due_date && t.due_date < todayStr && !isSuspended
                ? "overdue"
                : t.status;
            return (<TableRow key={t.id} className={cn("group transition-colors", isSuspended && t.status !== 'paid'
                  ...
                  <TableCell className="py-4 w-10">
                    <Checkbox checked={selectedIds.has(t.id)} onCheckedChange={(c) => {
                    setSelectedIds((prev) => {
                  ...
                  <TableCell className="py-4">
                    <div className="flex items-center gap-2">
                      {t.number_display && (<span className="text-[10px] font-mono-kasa font-bold text-muted-foreground bg-muted/40 border border-border/60 rounded px-1.5 py-0.5 shrink-0">
                          {t.number_display}
                        </span>)}
                      <button type="button" onClick={() => setEditingTx(t)} className="font-semibold text-sm text-left hover:text-primary hover:underline underline-offset-2 transition-colors cursor-pointer" title="Editar lançamento">
                        {(() => {
                    const dme = t.extra_demands;
                    const batch = t.dme_batches;
                    ...
                    return t.description;
                      ...
                      {isNaoOp && (<span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/30">
                          Não-op
                        </span>)}
                      {boletoByTx.get(t.id) && (() => {
                    const b = boletoByTx.get(t.id);
                    const isPaid = b.situacao === "RECEBIDO";
                    return (<button type="button" onClick={() => openBoletoPdf(b.id)} title={`Boleto Inter · ${b.situacao}`} className={cn("inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border transition-colors cursor-pointer", isPaid
                    ...
                    <div className="flex flex-col gap-0.5">
                      {t.is_internal ? (<>
                          ...
                          <InlineSupplierPicker transactionId={t.id} currentSupplierId={t.supplier_id} currentSupplierName={t.suppliers?.name || null}/>
                        </>) : ((() => {
                    const catName = (t.categorias_financeiras?.nome || t.category || "")
                        .toString()
                        .toLowerCase()
                        .normalize("NFD")
                        .replace(/[\u0300-\u036f]/g, "")
                    ...
                    if (isProLabore) {
                        return (<span className="text-[10px] text-foreground/40 italic">
                                Direcionado aos sócios
                              </span>);
                    }
                    const isFreelaCat = catName.includes("freelancer") ||
                        catName.includes("freela") ||
                        catName.includes("terceiriz");
                    return (<>
                              <InlineClientPicker transactionId={t.id} currentClientId={t.client_id} currentClientName={t.clients?.company || t.clients?.name || null} currentClientPhotoUrl={t.clients?.logo_url || null}/>
                              {t.type === "expense" && ((isFreelaCat || t.freelancer_id) ? (<InlineFreelancerPicker transactionId={t.id} currentFreelancerId={t.freelancer_id} currentFreelancerName={t.freelancer?.name || null} currentFreelancerPhotoUrl={t.freelancer?.photo_url || null}/>) : (<InlineSupplierPicker transactionId={t.id} currentSupplierId={t.supplier_id} currentSupplierName={t.suppliers?.name || null}/>))}
                            </>);
                })())}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="py-4">
                    <InlineCategoryPicker transactionId={t.id} currentCategoryId={t.category_id} transactionType={t.type}/>
                  </TableCell>
                  <TableCell className={cn("py-4 text-right font-semibold text-sm tabular-nums", typeColor)}>
                    <div className="flex flex-col items-end">
                      <span>{sign} {brl(previsto)}</span>
                      {t.clients?.financial_collection_status === 'suspended' && t.status !== 'paid' && (<span className="text-[9px] font-bold text-amber-500 uppercase">Cobrança Suspensa</span>)}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="py-4 text-right">
                    {hasDiff && t.clients?.financial_collection_status !== 'suspended' ? (<TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Badge className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold tabular-nums", diff > 0 ? "bg-orange-500 text-white" : "bg-emerald-500 text-white")}>
                              {diff > 0 ? "+" : ""}
                              {brl(diff)}
                            </Badge>
                          </TooltipTrigger>
                      ...
                      </TooltipProvider>) : (<span className="text-foreground/20 text-xs">—</span>)}
                  </TableCell>
                  <TableCell className="py-4 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <StatusBadge status={effectiveStatus} dueDate={t.due_date}/>
                      <div className="flex items-center gap-1.5">
                        <InternalControls transactionId={t.id} nfStatus={t.nf_status || "pendente"} boletoStatus={t.boleto_internal_status || "nao_se_aplica"}/>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="py-4">
                    <div className="flex items-center justify-end gap-1">
                      {(t.status === "pending" || t.status === "overdue") && (<TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => setBaixaTx(t)} className="h-8 w-8 rounded-lg text-emerald-600 hover:text-emerald-700 hover:bg-emerald-500/10">
                      ...
                      {t.type === "income" && t.client_id && (t.status === "pending" || t.status === "overdue") && !boletoByTx.get(t.id) && (<TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => setBoletoTx(t)} className="h-8 w-8 rounded-lg text-orange-600 hover:text-orange-700 hover:bg-orange-500/10">
                      ...
                      {t.status === "paid" && (<TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => setReciboTx(t)} className="h-8 w-8 rounded-lg text-blue-600 hover:text-blue-700 hover:bg-blue-500/10">
                      ...
                      {t.status === "paid" && (<TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => statusMut.mutate({ id: t.id, status: "pending" })} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" onClick={() => setEditingTx(t)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" onClick={() => setDeletingTx(t)} className="h-8 w-8 rounded-lg text-red-600 dark:text-red-400 hover:text-red-600 hover:bg-red-500/10">
        ...
        <div className="border-t border-border bg-muted/30 px-4 py-3 grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <TotalCell label="Saldo do Período" value={saldoPeriodo} className={saldoPeriodo >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}/>
        </div>
      </div>
      ...
      <TransactionFormDialog open={!!editingTx} onOpenChange={(o) => !o && setEditingTx(null)} transaction={editingTx}/>
      ...
      <BaixaDialog open={!!baixaTx} onOpenChange={(o) => !o && setBaixaTx(null)} transaction={baixaTx}/>
      <DeleteTransactionDialog open={!!deletingTx} onOpenChange={(o) => !o && setDeletingTx(null)} transaction={deletingTx}/>
      <EmitirBoletoDialog open={!!boletoTx} onOpenChange={(o) => !o && setBoletoTx(null)} transaction={boletoTx}/>
      <ReciboDialog open={!!reciboTx} onOpenChange={(o) => !o && setReciboTx(null)} transaction={reciboTx}/>
    </div>);
}
function TotalCell({ label, value, className }) {
...
function StatCard({ title, value, icon: Icon, color, bg, cardBg }) {
    return (<Card className={cn("shadow-sm overflow-hidden group hover:border-primary/50 transition-colors", cardBg || "bg-surface border-border")}>
      <CardContent className="p-4 flex flex-col justify-between h-full space-y-3">
        <div className="flex items-center justify-between">
          <div className={cn("size-8 rounded-xl flex items-center justify-center", bg || "bg-muted")}>
            <Icon className={cn("size-4", color)}/>
          </div>
          {title !== 'PARCELAS FUTURAS' && (<Badge variant="outline" className="text-[10px] font-mono-kasa text-foreground/30">Mês Atual</Badge>)}
        </div>
        <div>
...
function StatusBadge({ status, dueDate }) {
    ...
    return (<Badge variant="outline" className={cn("rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest border-2", cls)}>
      {label}
    </Badge>);
}
function InternalControls({ transactionId, nfStatus, boletoStatus }) {
    const queryClient = useQueryClient();
    const mutation = useMutation({
        mutationFn: (patch) => updateTransaction(transactionId, patch),
        onSuccess: () => {
