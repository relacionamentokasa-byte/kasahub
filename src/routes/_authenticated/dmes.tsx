  ...
  const { data: activeBatchesRaw = [] } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, public_token, status, total_value, consolidated_transaction_id, client_id, created_at, friendly_number, clients(name, company), dme_batch_items(extra_demand_id), transactions:consolidated_transaction_id(due_date)")
