...
async function fetchSaudeNegocio(refDate) {
    ...
    const { data: txData, error: txErr } = await supabase
        .from("transactions")
        .select("amount, valor_previsto, type, kind, nature, is_recurring, due_date, status, contract_id, proposal_id, client_id, categorias_financeiras(nome), contracts(type), proposals(contract_type), clients(financial_collection_status)")
    ...
    const isRecurringTx = (t) => {
        if (matchesKeyword(t.categorias_financeiras?.nome, MRR_KEYWORDS))
            return true;
        if (t.is_recurring === true)
            return true;
        const contractType = t.contracts?.type || t.proposals?.contract_type;
        if (contractType && String(contractType).toLowerCase().includes("recurring"))
            return true;
        return false;
    };
    const isAvulsoTx = (t) => {
        if (matchesKeyword(t.categorias_financeiras?.nome, AVULSO_KEYWORDS))
            return true;
    ...
    const getAmount = (t) => Number(Number(t.valor_previsto) > 0 ? t.valor_previsto : t.amount) || 0;
    ...
    const clientesFaturadosMes = new Set(incomes.map((t) => t.client_id).filter(Boolean)).size;
    ...
    (activeProjects || []).forEach((p) => {
        if (p.client_id)
            clientesAtivosSet.add(p.client_id);
    });
    incomes.forEach((t) => {
        if (t.client_id)
            clientesAtivosSet.add(t.client_id);
    });
    ...
    const faturadoAnual = (yearIncomes || [])
        ...
        .reduce((acc, t) => acc + (Number(t.paid_value ?? t.valor_real ?? t.amount) || 0), 0);
    return {
        mrr,
        avulsa,
        receitaEfetivada,
...
export function SaudeNegocioSection() {
    ...
    const saveMeta = useMutation({
        ...
        onError: (e) => toast.error(e?.message || "Erro ao salvar meta"),
    });
    const commit = () => {
    ...
    return (<div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
        <DashboardKPI icon={TrendingUp} label="MRR" value={brl(mrr)} subValue="Receita recorrente" color="emerald-500"/>
        <DashboardKPI icon={Zap} label="Receita Avulsa (Mês)" value={brl(avulsa)} subValue="Jobs pontuais" color="amber-500"/>
        <DashboardKPI icon={Receipt} label="Ticket Médio" value={brl(clientesFaturadosMes > 0 ? (mrr + avulsa) / clientesFaturadosMes : 0)} subValue="(MRR + Avulsa) ÷ clientes que faturaram no mês" color="indigo-500"/>
        <DashboardKPI icon={Users} label="Clientes Ativos" value={clientesAtivos} subValue="Recorrentes + avulsos do mês (distintos)" color="blue-500"/>
        <DashboardKPI icon={FileText} label="Propostas Pendentes" value={propostasPendentes} subValue="Enviadas e rascunhos" color="primary"/>
      </div>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-foreground/40 flex items-center gap-1.5 flex-wrap">
                ...
                {editing ? (<Input ref={inputRef} type="number" inputMode="decimal" value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={commit} onKeyDown={(e) => {
                if (e.key === "Enter")
                    commit();
                if (e.key === "Escape")
                    setEditing(false);
            }} className="h-6 w-32 text-xs px-2" placeholder="0.00"/>) : (<button type="button" onClick={() => setEditing(true)} className="inline-flex items-center gap-1 hover:text-primary transition-colors">
