...
export async function createJob(input: Database["public"]["Tables"]["jobs"]["Insert"] & { period?: string | null, job_type?: string | null }) {
  ...
  if (!input.service_id && !input.editorial_post_id && !input.launch_product_id) throw new Error("Um job deve estar vinculado a um serviço, post editorial ou produto.");
